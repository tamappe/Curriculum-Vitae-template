...
メインタスクはモバイルアプリで使われるライブラリのSDKの保守・運用になります。  

Android SDKをメインに担当しながらiOSでもリソースが足りない時に担当しました。  
SDKの役割はログ送信です。  
アーキテクチャはクリーンアーキテクチャを採用したSDKで、新規機能追加する時は既存のアーキテクチャに従う方針を採用。  
サービス側から影響や問題の問い合わせがきたときに原因の調査、改修することがメインです。  
リリースした新規機能はチームメンバーに共有するためドキュメントに残す必要があるため技術者用のドキュメントを残すスキルが向上。  

ドキュメントの豊富なプロジェクトですが品質管理やリリース前の検証の手順が不十分なことに気づきました。  
それを補完するドキュメントを新たに作成するなど、品質を担保するためのフローを整備するなどの改善を図りました。

直近では、ユーザーからの問い合わせで一部で送信できていないログがあるなどの不具合を調査するタスクを行いました。  
クライアント側からAPIにデータを渡せていないケースが存在するとのことでクライアントサイドでログの送信ができているかの形跡を
テキストファイルとして残せるようにする仕込みを開発してクライアントサイドから調査できるようにして解決しました。
